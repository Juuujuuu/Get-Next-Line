...
char	*readline(int fd, char *buf)
{
	...
	static char	*save;// variable statique qui conserve ce qui a été lu en trop 
	...
	if (retread > 0)
	{
		...
		tmp[i] = '\0'; // On signale la fin de la copie à tmp en mettant \0
		if (buf[i] == '\n' && i <= retread) // si on arrive à un retour à la ligne
		{
